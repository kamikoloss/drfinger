...
func _draw() -> void:
    var step_height := bar_height / (beats_per_bar * steps_per_beat)
    var lane_width := size.x / lane_types.size()
    # 背景色 (全体)
    ...
    for i in lane_types.size():
        ...
        draw_rect(Rect2(Vector2(lane_width * i, 0), Vector2(lane_width, size.y)), lane_color)
    # 横線
    var line_y := 0.0
